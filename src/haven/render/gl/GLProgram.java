...
public class GLProgram implements Disposable {
	...
	public static class ShaderOb extends GLObject implements BGL.ID {
		...
		public void create(GL3 gl) {
			 ...
			 * be ASCII, of course, but it would be interesting to know, so to speak.
			 */
	...
	public class ProgOb extends GLObject implements BGL.ID {
		...
		public ProgOb(GLEnvironment env, ShaderOb... shaders) {
			...
			for (Map.Entry<Uniform, String> uni : GLProgram.this.unifnms.entrySet()) {
				...
				/*
				 * XXX: This should work with samplers in compound uniforms, but that requires
				 * support in preparing them as well, so wait until actually used for anything.
				 */
		...
		public class UniformID extends VarID {
			...
			public int glid() {
				if (id < 0)
					throw (new UnknownExternException("Uniform not resolvable in program: " + name, GLProgram.this, "uniform",
							name));
				return (id);
			}
		}
	...
	public ProgOb glid() {
		...
		if ((glp = this.glp) == null) {
			synchronized (this) {
				...
				if ((glp = this.glp) == null) {
					glp = new ProgOb(env, new ShaderOb(env, GL3.GL_VERTEX_SHADER, vsrc),
