...
public interface Rendered {
	...
	public static class ScreenQuad implements Rendered, RenderTree.Node {
		public static final Model data = new Model(Model.Mode.TRIANGLE_STRIP,
				new VertexArray(new VertexArray.Layout(
						new VertexArray.Layout.Input(Ortho2D.pos, new VectorFormat(2, NumberFormat.SNORM8), 0, 0, 4),
						new VertexArray.Layout.Input(Tex2D.texc, new VectorFormat(2, NumberFormat.SNORM8), 0, 2, 4)),
						new VertexArray.Buffer(16, DataBuffer.Usage.STATIC, DataBuffer.Filler.of(new byte[] { -127,
								-127, 0, 0, 127, -127, 127, 0, -127, 127, 0, 127, 127, 127, 127, 127, }))),
				null, 0, 4);
		...
		public ScreenQuad(boolean invert) {
			...
			if (invert)
				...
				vxf = new Ortho2D(-1, 1, 1, -1);
			state = Pipe.Op.compose(States.maskdepth, States.Depthtest.none,
					new States.Facecull(States.Facecull.Mode.NONE), vxf);
		}
