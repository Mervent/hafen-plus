	...
	public VertexArray(Layout fmt, Buffer... bufs) {
		if (bufs.length != fmt.nbufs)
			throw (new IllegalArgumentException(
					String.format("Vertex layout requires %d buffers, only given %d", fmt.nbufs, bufs.length)));
		this.fmt = fmt;
		this.bufs = bufs;
	}
	...
	public int num() {
		...
		for (int i = 0; i < fmt.inputs.length; i++) {
			...
			if (bufs[inp.buf].size < (inp.offset + inp.el.size()))
				return (0);
			int n = 1 + ((bufs[inp.buf].size - (inp.offset + inp.el.size()))
					/ ((inp.stride != 0) ? inp.stride : inp.el.size()));
			if ((ret < 0) || (n < ret))
				ret = n;
		}
		return (ret);
	...
	public static class Layout {
		...
		public static class Input {
			...
			public String toString() {
				return (String.format("#<input %s(%s = %sx%s) %d(%d@%d)>", tgt.name, tgt.type, el.cf, el.nc, buf,
						stride, offset));
			}
		}
