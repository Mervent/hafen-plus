...
public class Test implements GLEventListener, KeyListener {
	...
	static {
		...
		tex = new Texture2D.Sampler2D(new Texture2D(texsz.x, texsz.y, DataBuffer.Usage.STATIC,
				new VectorFormat(4, NumberFormat.UNORM8), new VectorFormat(4, NumberFormat.UNORM8), (img, env) -> {
					if (img.level != 0)
						return (null);
	...
	final Texture2D.Sampler2D tbuf = new Texture2D.Sampler2D(
			new Texture2D(512, 512, DataBuffer.Usage.STATIC, new VectorFormat(4, NumberFormat.UNORM8), null));
	final Texture2D dbuf = new Texture2D(512, 512, DataBuffer.Usage.STATIC, Texture.DEPTH,
	...
	private void display(Render g) {
		...
		g.clear(fb, 1.0);
		{
			...
			VertexArray.Layout fmt = new VertexArray.Layout(
					new VertexArray.Layout.Input(Ortho2D.pos, new VectorFormat(2, NumberFormat.FLOAT32), 0, 0, 24),
					new VertexArray.Layout.Input(VertexColor.color, new VectorFormat(4, NumberFormat.FLOAT32), 0, 8,
							24));
			float[] data = { 50, 50, 1, 0, 0, 1, 25, 86, 0, 1, 0, 1, 75, 86, 0, 0, 1, 1, };
...

		{
			...
			vc.apply(pipe);
			VertexArray.Layout fmt = new VertexArray.Layout(
					new VertexArray.Layout.Input(Ortho2D.pos, new VectorFormat(2, NumberFormat.SINT16), 0, 0, 8),
					new VertexArray.Layout.Input(VertexColor.color, new VectorFormat(4, NumberFormat.UNORM8), 0, 4, 8));
			byte[] data = { (byte) 150, 0, 50, 0, (byte) 255, 0, 0, (byte) 255, (byte) 125, 0, 86, 0, 0, (byte) 255, 0,
					(byte) 255, (byte) 175, 0, 86, 0, 0, 0, (byte) 255, (byte) 255, };
			g.draw(pipe, new Model(Model.Mode.TRIANGLES, new VertexArray(fmt,
					new VertexArray.Buffer(data.length, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))), null,
					0, 3));
		}

		{
			...
			VertexArray.Layout fmt = new VertexArray.Layout(
					new VertexArray.Layout.Input(Ortho2D.pos, new VectorFormat(2, NumberFormat.SINT16), 0, 0, 6),
					new VertexArray.Layout.Input(Tex2D.texc, new VectorFormat(2, NumberFormat.UNORM8), 0, 4, 6));
			byte[] data = { (byte) 100, 0, (byte) 228, 0, 0, (byte) 255, (byte) 100, 0, 100, 0, 0, 0, (byte) 228, 0,
					(byte) 228, 0, (byte) 255, (byte) 255, (byte) 225, 0, 100, 0, (byte) 255, 0, };
			g.draw(pipe, new Model(Model.Mode.TRIANGLE_STRIP, new VertexArray(fmt,
					new VertexArray.Buffer(data.length, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))), null,
					0, 4));
		}

		{
			...
			float[] data = { 100, 612, 0, 0, 100, 100, 0, 1, 612, 612, 1, 0, 612, 100, 1, 1, };
			g.draw(pipe, new Model(Model.Mode.TRIANGLE_STRIP, new VertexArray(fmt,
					new VertexArray.Buffer(data.length * 4, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))),
					null, 0, 4));
		}
	}
	...
	public void reshape(GLAutoDrawable wnd, int x, int y, int w, int h) {
		...
		// base.prep(new FragColor(new Texture2D(512, 512, DataBuffer.Usage.STATIC, new
		// VectorFormat(4, NumberFormat.UNORM8), null).image(0)));
		// base.prep(new DepthBuffer(new Texture2D(512, 512, DataBuffer.Usage.STATIC,
		// Texture.DEPTH, new VectorFormat(1, NumberFormat.FLOAT32), null).image(0)));
		this.shape = area;
		this.base = base;
	}
