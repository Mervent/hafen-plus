...
public class CloudShadow extends State {
	...
	private static final ShaderMacro shader = prog -> {
		...
		final ValBlock.Value shval = prog.fctx.uniform.new Value(FLOAT) {
			public Expression root() {
				Expression tc = add(
						mul(add(pick(Homo3D.fragmapv.ref(), "xy"), mul(pick(Homo3D.fragmapv.ref(), "z"), cdir.ref())),
								cscl.ref()),
						mul(cvel.ref(), Glob.FrameInfo.globtime()));
		...
		ph.dolight.mod(new Runnable() {
			public void run() {
				ph.dolight.dcalc.add(new If(eq(MapView.amblight_idx.ref(), ph.dolight.i),
						stmt(amul(ph.dolight.dl.tgt, shval.ref()))), ph.dolight.dcurs);
			}
		}, 0);
	};
	...
	public boolean equals(CloudShadow that) {
		return ((this.tex == that.tex) && Utils.eq(this.dir, that.dir) && Utils.eq(this.vel, that.vel)
				&& (this.scale == that.scale) && (this.cmin == that.cmin) && (this.cmax == that.cmax)
				&& (this.rmin == that.rmin) && (this.rmax == that.rmax));
	}
