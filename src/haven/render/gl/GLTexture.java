		...
		while (true) {
			...
			if (fbo == last)
				throw (new RuntimeException(
						String.format("FBO %s somehow not unregistering from %s when disposing", fbo, this)));
			fbo.dispose();
			last = fbo;
		}
		super.dispose0();
				...
				case 1:
					switch (fmt.cf) {
						case UNORM8:
							return (GL3.GL_R8);
						case SNORM8:
							return (GL3.GL_R8_SNORM);
						case UNORM16:
							return (GL3.GL_R16);
						case SNORM16:
							return (GL3.GL_R16_SNORM);
						case FLOAT16:
							return (GL3.GL_R16F);
						case DEPTH:
				...
				case 2:
					switch (fmt.cf) {
						case UNORM8:
							return (GL3.GL_RG8);
						case SNORM8:
							return (GL3.GL_RG8_SNORM);
						case UNORM16:
							return (GL3.GL_RG16);
						case SNORM16:
							return (GL3.GL_RG16_SNORM);
						case FLOAT16:
							return (GL3.GL_RG16F);
					}
				case 3:
					switch (fmt.cf) {
						case UNORM8:
							return (GL.GL_RGB8);
						case SNORM8:
							return (GL3.GL_RGB8_SNORM);
						case UNORM16:
							return (GL3.GL_RGB16);
						case SNORM16:
							return (GL3.GL_RGB16_SNORM);
						case FLOAT16:
							return (GL3.GL_RGB16F);
					}
				case 4:
					switch (fmt.cf) {
						case UNORM8:
							return (GL.GL_RGBA8);
						case SNORM8:
							return (GL3.GL_RGBA8_SNORM);
						case UNORM16:
							return (GL3.GL_RGBA16);
						case SNORM16:
							return (GL3.GL_RGBA16_SNORM);
						case FLOAT16:
		...
		if (ifmt.cf == NumberFormat.DEPTH) {
			if (efmt.nc != 1)
				throw (new IllegalArgumentException(
						String.format("externalformat components != 1 for depth texture: %s", efmt)));
			return (GL3.GL_DEPTH_COMPONENT);
		}
	...
	public static class Tex3D extends GLTexture {
		...
		public Tex3D(GLEnvironment env, Texture3D data, FillBuffers.Array[] pixels) {
			...
			env.prepare((GLRender g) -> {
				...
				bind(gl);
				if (pixels[0] != null)
					gl.glTexImage3D(GL3.GL_TEXTURE_3D, 0, ifmt, data.w, data.h, data.d, 0, pfmt, pnum,
							ByteBuffer.wrap(pixels[0].data));
				else
				...
				for (int i = 1; i < pixels.length; i++) {
					if (pixels[i] != null) {
						Image<?> img = data.image(i);
						gl.glTexImage3D(GL.GL_TEXTURE_2D, i, ifmt, img.w, img.h, img.d, 0, pfmt, pnum,
								ByteBuffer.wrap(pixels[i].data));
