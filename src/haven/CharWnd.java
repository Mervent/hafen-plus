...
public class CharWnd extends Window {
	...
	public static final Text.Furnace catf = new BlurFurn(
			new TexFurn(new Text.Foundry(Text.fraktur, 25).aa(true), Window.ctex), 3, 2, new Color(96, 48, 0));
	public static final Text.Furnace failf = new BlurFurn(
			new TexFurn(new Text.Foundry(Text.fraktur, 25).aa(true), Resource.loadimg("gfx/hud/fontred")), 3, 2,
			new Color(96, 48, 0));
	...
	public static class FoodMeter extends Widget {
		...
		public Object tooltip(Coord c, Widget prev) {
			if (rtip == null) {
				...
				cur = ItemInfo.catimgs(0, cur, Text
						.render(String.format("Total: %s/%s", Utils.odformat2(sum, 2), Utils.odformat(cap, 2))).img);
				rtip = new TexI(cur);
			}
			return (rtip);
		}
	...
	public static class GlutMeter extends Widget {
		...
		public Object tooltip(Coord c, Widget prev) {
			if (rtip == null) {
				rtip = RichText.render(String.format("%s: %d%%\nFood efficacy: %d%%", lbl, Math.round((lglut) * 100),
						Math.round(gmod * 100)), -1).tex();
			}
			return (rtip);
		}
	}
			...
			parent.add(new Widget() {
				...
				public void draw(GOut g) {
					if (img != null) {
						 ...
						 * msg.sz().x) / 2, Math.max(img.sz().y, title.sz().y) + 25));
						 */
						int y = 0;
	...
	public class CredoGrid extends Scrollport {
		...
		private void update() {
			...
			if (pcr != null) {
				...
				cont.add(new Label(String.format("Quest: %d/%d", pcql, pcqlt), prsf), pcrim.c.x + pcrim.sz.x + 5,
						y + 20);
	...
	public CharWnd(Glob glob) {
		...
		Tabs.Tab skills;
		{
			...
			y += 35;
			final LoadingTextBox info = skills.add(new LoadingTextBox(new Coord(attrw, 260), "", ifnd),
	...
	public void addchild(Widget child, Object... args) {
		...
		if (place == "study") {
			...
			Widget inf = sattr.add(new StudyInfo(new Coord(attrw - 150, child.sz.y), child),
					new Coord(260 + 150, child.c.y).add(wbox.btloff().x, 0));
