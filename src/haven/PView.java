...
public abstract class PView extends Widget {
	...
	/*
	 * XXX? Remove standard clearing and assume implementations to add explicit
	 * clearing slot?
	 */
	...
	private GOut resolveout(GOut def, PostProcessor next) {
		...
		if (next.buf == null) {
			Texture2D tex = new Texture2D(fragcol.sz(), DataBuffer.Usage.STATIC,
					new VectorFormat(4, NumberFormat.UNORM8), null);
	...
	protected void basic() {
		basic(id_fb, p -> {
			...
			if ((depth == null) || !depth.sz().equals(fb.sz)) {
				if (depth != null)
					depth.dispose();
				depth = new Texture2D(fb.sz, DataBuffer.Usage.STATIC, Texture.DEPTH,
		...
		basic(id_misc,
				Pipe.Op.compose(
						new States.Blending(States.Blending.Function.ADD, States.Blending.Factor.SRC_ALPHA,
								States.Blending.Factor.INV_SRC_ALPHA, States.Blending.Function.MAX,
								States.Blending.Factor.SRC_ALPHA, States.Blending.Factor.INV_SRC_ALPHA),
						new States.Depthtest(States.Depthtest.Test.LE), new States.Facecull()));
		lights();
	}
