...
public class JOGLPanel extends GLCanvas implements Runnable, UIPanel, Console.Directory, UI.Context {
	...
	private void drawtooltip(UI ui, GOut g) {
		...
		if (prevtooltip != null) {
			/*
			 * Tooltip disposal the same frame seems to have a tendency to cause some kind
			 * of weird CPU/GPU sync point. Arguably this should be handled by the rendering
			 * system somehow, but it's unclear what the actual root cause is, and this is a
			 * cheap fix.
			 */
			prevtooltip.dispose();
			prevtooltip = null;
	...
	private void drawstats(UI ui, GOut g, GLRender buf) {
		int y = g.sz().y - 190;
		FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "FPS: %d (%d%%, %d%% idle, latency %d)", fps,
				(int) (uidle * 100.0), (int) (ridle * 100.0), framelag);
		Runtime rt = Runtime.getRuntime();
		long free = rt.freeMemory(), total = rt.totalMemory();
		if (free < prevfree)
			framealloc = ((prevfree - free) + (framealloc * 19)) / 20;
		prevfree = free;
		FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "Mem: %,011d/%,011d/%,011d/%,011d (%,d)", free, total - free,
				total, rt.maxMemory(), framealloc);
		...
		if ((map != null) && (map.back != null)) {
			...
			// FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "Click: Map: %s, Obj: %s",
			// map.clmaplist.stats(), map.clobjlist.stats());
		}
		if (ui.sess != null)
			FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "Async: L %s, D %s", ui.sess.glob.loader.stats(),
					Defer.gstats());
		else
			FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "Async: D %s", Defer.gstats());
		int rqd = Resource.local().qdepth() + Resource.remote().qdepth();
		if (rqd > 0)
			FastText.aprintf(g, new Coord(10, y -= 15), 0, 1, "RQ depth: %d (%d)", rqd,
					Resource.local().numloaded() + Resource.remote().numloaded());
	}
