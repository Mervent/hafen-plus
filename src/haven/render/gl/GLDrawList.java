...
public class GLDrawList implements DrawList {
	...
	abstract class DepSetting extends Setting {
		...
		Pipe compstate() {
			...
			return (new Pipe() {
				...
				/*
				 * Not entirely clear whether these should even be implemented. Evaluate in case
				 * they are ever used.
				 */
				public Pipe copy() {
	...
	class SlotRender implements Render {
		...
		public void draw(Pipe st, Model mod) {
			...
			if (GLVertexArray.ephemeralp(mod)) {
				...
				if (mod.ind == null) {
					...
					if (mod.ninst == 1)
						gl.glDrawElements(GLRender.glmode(mod.mode), mod.n, GLRender.glindexfmt(mod.ind.fmt),
								mod.f * mod.ind.fmt.size);
					else
						gl.glDrawElementsInstanced(GLRender.glmode(mod.mode), mod.n, GLRender.glindexfmt(mod.ind.fmt),
								mod.f * mod.ind.fmt.size, mod.ninst);
				}
				slot.main = gl;
			}
			done = true;
		...
		/*
		 * Somewhat unclear whether DrawList should implement these. Just implement it
		 * if and when it turns out to be reasonable.
		 */
	...
	public void update(Slot<? extends Rendered> slot) {
		synchronized (this) {
			/*
			 * Handle exceptions from DrawSlot construction before removing previous slot.
			 */
			DrawSlot dslot = new DrawSlot(slot);
			remove(slot);
			dslot.insert();
