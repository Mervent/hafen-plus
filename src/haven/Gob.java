...
public class Gob implements RenderTree.Node, Sprite.Owner, Skeleton.ModOwner {
	...
	public void updwait(Runnable callback, Consumer<Waitable.Waiting> reg) {
		/*
		 * Caller should probably synchronize on this already for a call like this to
		 * even be meaningful, but just in case.
		 */
		synchronized (this) {
			if (updwait == null)
	...
	public static class DataLoading extends Loading {
		...
		/*
		 * It would be assumed that the caller has synchronized on gob while creating
		 * this exception.
		 */
