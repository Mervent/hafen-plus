...
public class FboState extends GLState {
	...
	public void applydbufs(BGL gl) {
		...
		/*
		 * Just set any valid read-buffer for completeness checking. GLRender switches
		 * to the correct read-buffer when needed.
		 */
		if (fbo == null) {
			if (env.nilfbo_id == 0)
	...
	public static FboState make(GLEnvironment env, Object depth, Object[] fvals) {
		...
		if (!any) {
			...
			throw (new IllegalArgumentException(String.format("Illegal framebuffer configuration: depth=%s, colors=%s",
					depth, Arrays.asList(fvals))));
		}
	}
