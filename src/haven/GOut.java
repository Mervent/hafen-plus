...
public class GOut {
	public static final VertexArray.Layout vf_pos = new VertexArray.Layout(
			new VertexArray.Layout.Input(Ortho2D.pos, new VectorFormat(2, NumberFormat.FLOAT32), 0, 0, 8));
	public static final VertexArray.Layout vf_tex = new VertexArray.Layout(
	...
	public void drawp(Model.Mode mode, float[] data, int n) {
		out.draw(cur2d, new Model(mode, new VertexArray(vf_pos,
				new VertexArray.Buffer(data.length * 4, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))), null,
				0, n));
	}
	...
	public void drawt(Model.Mode mode, float[] data, int n) {
		out.draw(cur2d, new Model(mode, new VertexArray(vf_tex,
				new VertexArray.Buffer(data.length * 4, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))), null,
				0, n));
	}
	...
	public void line(Coord c1, Coord c2, double w) {
		...
		float[] data = { c1.x + tx.x + 0.5f, c1.y + tx.y + 0.5f, c2.x + tx.x + 0.5f, c2.y + tx.y + 0.5f };
	...
	/*
	 * XXXRENDER public void ftexrect(Coord ul, Coord sz, GLState s, float tl, float
	 * tt, float tr, float tb) { ul = tx.add(ul); Coord br = ul.add(sz); Coord ult =
	 * new Coord(0, 0); Coord brt = new Coord(sz); if(ul.x < this.ul.x) { ult.x +=
	 * this.ul.x - ul.x; ul.x = this.ul.x; } if(ul.y < this.ul.y) { ult.y +=
	 * this.ul.y - ul.y; ul.y = this.ul.y; } if(br.x > this.ul.x + this.sz.x) {
	 * brt.x -= br.x - (this.ul.x + this.sz.x); br.x = this.ul.x + this.sz.x; }
	 * if(br.y > this.ul.y + this.sz.y) { brt.y -= br.y - (this.ul.y + this.sz.y);
	 * br.y = this.ul.y + this.sz.y; } if((ul.x >= br.x) || (ul.y >= br.y)) return;
	 *
	 * st.set(cur2d); state(s); apply();
	 *
	 * float l = tl + ((tr - tl) * ((float)ult.x) / ((float)sz.x)); float t = tt +
	 * ((tb - tt) * ((float)ult.y) / ((float)sz.y)); float r = tl + ((tr - tl) *
	 * ((float)brt.x) / ((float)sz.x)); float b = tt + ((tb - tt) * ((float)brt.y) /
	 * ((float)sz.y)); gl.glBegin(GL2.GL_QUADS); gl.glTexCoord2f(l, 1 - t);
	 * gl.glVertex2i(ul.x, ul.y); gl.glTexCoord2f(r, 1 - t); gl.glVertex2i(br.x,
	 * ul.y); gl.glTexCoord2f(r, 1 - b); gl.glVertex2i(br.x, br.y);
	 * gl.glTexCoord2f(l, 1 - b); gl.glVertex2i(ul.x, br.y); gl.glEnd(); checkerr();
	 * }
	 *
	 * public void ftexrect(Coord ul, Coord sz, GLState s) { ftexrect(ul, sz, s, 0,
	 * 0, 1, 1); }
	 */
	...
	public void rect2(Coord ul, Coord br) {
		...
		br = br.add(tx);
		float h = 0.5f;
		float[] data = { ul.x + h, ul.y + h, br.x + h, ul.y + h, br.x + h, br.y + h, ul.x + h, br.y + h, ul.x + h,
				ul.y + h };
	...
	public static void getimage(Render g, Pipe state, FragData buf, Area area, Consumer<BufferedImage> cb) {
		g.pget(state, buf, area, new VectorFormat(4, NumberFormat.UNORM8), data -> {
			...
			data.get(pbuf);
			WritableRaster raster = Raster.createInterleavedRaster(new DataBufferByte(pbuf, pbuf.length), sz.x, sz.y,
	...
	public static void getimage(Render g, Texture.Image<?> img, Consumer<BufferedImage> cb) {
		if (img.tex.ifmt.cf == NumberFormat.DEPTH) {
			g.pget(img, new VectorFormat(1, NumberFormat.FLOAT32), data -> {
				...
				WritableRaster raster = Raster.createInterleavedRaster(new DataBufferByte(pbuf, pbuf.length), sz.x,
						sz.y, 4 * sz.x, 4, new int[] { 0, 1, 2, 3 }, null);
				cb.accept(new BufferedImage(TexI.glcm, raster, false, null));
			});
		} else {
