...
public class RenderTree implements RenderList.Adapter {
	...
	public static class DepInfo {
		...
		/*
		 * public boolean defequal(DepInfo that) { if(that.def.length < this.def.length)
		 * return(that.defequal(this)); if(this.ndef != that.ndef) return(false); int i;
		 * for(i = 0; i < this.def.length; i++) { if(this.def[i] != that.def[i])
		 * return(false); } for(; i < that.def.length; i++) { if(that.def[i])
	...
	static class TreeSlot implements Slot {
		...
		private void remrdep(int stidx, TreeSlot rdep) {
			if ((rdeps == null) || (rdeps.length <= stidx) || (rdeps[stidx] == null) || !rdeps[stidx].remove(rdep))
		...
		private void checklockdeps() {
			...
			for (int i = 0; i < deps.length; i++) {
				if ((deps[i] != null) && !deps[i].stlock)
					throw (new RuntimeException(
							String.format("locked state depends on non-locked state (node: %s)", node)));
			}
		}
