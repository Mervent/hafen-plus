...
public class GLRender implements Render, Disposable {
		...
		if (GLVertexArray.ephemeralp(data)) {
			...
			if (ne > 0) {
				...
				if (ne == 1) {
					...
					gl.glBufferData(GL.GL_ARRAY_BUFFER, data.va.bufs[sbuf].size(), ByteBuffer.wrap(((HeapBuffer) bufs[sbuf]).buf),
							GL3.GL_STREAM_DRAW);
					offsets[sbuf] = 0;
				} else {
					int sz = 0;
			...
			for (int i = 0; i < data.va.fmt.inputs.length; i++) {
				...
				gl.glVertexAttribPointer(enable[i], attr.el.nc, glattribfmt(attr.el.cf), glattribnorm(attr.el.cf), attr.stride,
						attr.offset + off);
			}
			...
			if (data.ind == null) {
				...
				if (data.ind.usage == EPHEMERAL) {
					...
					gl.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, data.ind.size(), ByteBuffer.wrap(((HeapBuffer) indo).buf),
							GL3.GL_STREAM_DRAW);
				} else {
				...
				if (data.ninst == 1)
					...
					gl.glDrawElementsInstanced(glmode(data.mode), data.n, glindexfmt(data.ind.fmt), data.f * data.ind.fmt.size,
							data.ninst);
			}
		} else {
	...
	public <T extends DataBuffer> void update(T buf, DataBuffer.PartFiller<? super T> fill, int from, int to) {
		...
		/*
		 * XXX: There is some case to be made that this whole implementation should be
		 * able to interact better with stream-buffers, but I'm not totally sure how.
		 */
		if (buf instanceof Model.Indices) {
			Model.Indices ibuf = (Model.Indices) buf;
	...
	public void pget(Pipe pipe, FragData buf, Area area, VectorFormat fmt, Consumer<ByteBuffer> callback) {
		...
		/*
		 * gl.glPixelStorei(GL.GL_PACK_ALIGNMENT, 1); gl.glReadBuffer(fc.dbufs[n]);
		 * gl.bglSubmit(cgl -> { ByteBuffer data = ByteBuffer.wrap(new byte[fmt.size() *
		 * area.area()]); cgl.glReadPixels(area.ul.x, gly, sz.x, sz.y,
		 * GLTexture.texefmt1(fmt, fmt), GLTexture.texefmt2(fmt, fmt), data);
		 * GLException.checkfor(cgl); data.rewind(); callback.accept(data); });
		 */
		...
		gl.bglCreate(new GLFence(env, cgl -> {
			...
			data.rewind();
			/*
			 * XXX: It's not particularly nice to do the flipping on the dispatch thread,
			 * but OpenGL does not seem to offer any GPU-assisted flipping.
			 */
			int el = fmt.size();
	...
	public void pget(Texture.Image img, VectorFormat fmt, Consumer<ByteBuffer> callback) {
		...
		if (img.tex instanceof Texture2D) {
			...
			if (img.tex.ifmt.cf == NumberFormat.DEPTH) {
				if (fmt.nc != 1)
					throw (new IllegalArgumentException(
							String.format("externalformat components != 1 for depth texture: %s", fmt)));
				gl.glGetTexImage(GL.GL_TEXTURE_2D, img.level, GL3.GL_DEPTH_COMPONENT, GLTexture.texefmt2(fmt, fmt), 0);
			} else {
				gl.glGetTexImage(GL.GL_TEXTURE_2D, img.level, GLTexture.texefmt1(fmt, fmt, null), GLTexture.texefmt2(fmt, fmt),
						0);
			}
			tex.unbind(gl);
		} else {
