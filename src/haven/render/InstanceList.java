...
public class InstanceList implements RenderList<Rendered>, RenderList.Adapter, Disposable {
	...
	private static class InstKey {
		...
		final Pipe[] ust;
		/*
		 * It may be argued that instids should be compared by equals rather than by
		 * identity, if need be.
		 */
	...
	private static class InstanceState extends BufPipe {
		...
		public String toString() {
			...
			for (int i = 0; i < mask.length; i++) {
				...
				if (i > 0)
					buf.append(", ");
				buf.append(String.format("%d(%s)=%s", id, State.Slot.byid(id).scl.getSimpleName(),
						get(State.Slot.byid(id))));
			}
			buf.append("]");
			return (buf.toString());
		}
	...
	private class InstancedSlot implements RenderList.Slot<Rendered>, InstanceBatch {
		...
		class Instance {
			...
			void update(Pipe group, int[] mask) {
				...
				/*
				 * XXX: There should be a way to only update the relevant states, by mask.
				 */
				iupdate(idx);
			}
		}
				...
				if (i > 0) {
					...
					if (InstanceState.compare(this.ist, test) != 0)
						throw (new RuntimeException(String
								.format("instantiation-IDs not yet implemented (states=%s vs %s)", this.ist, test)));
				}
			}
			this.insts = insts;
		...
		Instance remove(Instance inst) {
			...
			if (insts[ri] != inst)
				throw (new AssertionError());
			/*
			 * De-instancify when ni goes from 2 to 1? It's not *obviously* better to do so,
			 * and if the slot has once been instancified, it's probably not unreasonable to
			 * expect it to become so again in the future, in which case the updating
		...
		void update(Slot<? extends Rendered> ns) {
			/*
			 * XXX? Is this really necessary? Can't I just iupdate this? Also, if it is
			 * necessary, doesn't add() run the risk of throwing exceptions after remove()?
			 */
	...
	public void add(Slot<? extends Rendered> slot) {
		...
		if (!key.valid()) {
			/*
			 * XXX: Slots excluded in this manner aren't registered for in-pipe updates
			 * which might bring them back into instantiation. I don't really foresee that
			 * happening, but it is also perhaps not so purely theoretical as to be of
			 * purely academic interest. It shouldn't technically break anything, however;
			 * it should just mean that they can't be re-instantiated if their instid only
			 * changes in-pipe.
			 */
			cladd(slot);
			ninvalid++;
	...
	public void remove(Slot<? extends Rendered> slot) {
		...
		synchronized (this) {
			...
			if (key == null) {
				...
				/*
				 * XXX: Register a marker object in uslotmap and bring back the above assertion?
				 */
				ninvalid--;
				clremove(slot);
