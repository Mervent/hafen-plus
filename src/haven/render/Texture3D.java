...
public class Texture3D extends Texture {
	...
	public Texture3D(int w, int h, int d, DataBuffer.Usage usage, VectorFormat ifmt, VectorFormat efmt,
			DataBuffer.Filler<? super Image> init) {
		super(usage, ifmt, efmt, init);
		if ((w < 0) || (h < 0) || (d < 0))
			throw (new IllegalArgumentException(
					String.format("Texture sizes must be non-negative, not (%d, %d, %d)", w, h, d)));
		this.w = w;
		this.h = h;
		this.d = d;
	...
	public Texture3D(int w, int h, int d, DataBuffer.Usage usage, VectorFormat ifmt,
			DataBuffer.Filler<? super Image> init) {
	...
	public Image<Texture3D> image(int level) {
		...
		if (level > 0) {
			...
			if (((w >> level) == 0) && ((h >> level) == 0) && ((d >> level) == 0))
				throw (new IllegalArgumentException(
						String.format("Invalid mipmap level %d for (%d, %d, %d) texture", level, w, h, d)));
		}
		...
		return (new AbstractCollection<Image<Texture3D>>() {
			public int size() {
				if ((w == 0) || (h == 0) || (d == 0)) {
					...
					return (Math.max(Math.max(Integer.numberOfTrailingZeros(w), Integer.numberOfTrailingZeros(h)),
							Integer.numberOfTrailingZeros(d)) + 1);
				} else {
					return (1);
				}
			}
