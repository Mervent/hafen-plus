...
public class Homo3D {
	...
	public static Value vertedir(final VertexContext vctx) {
		return (vctx.mainvals.ext(vertedir_id, () -> vctx.mainvals.new Value(VEC3, new Symbol.Gen("edir")) {
			public Expression root() {
	...
	public static Value fragedir(final FragmentContext fctx) {
		return (fctx.mainvals.ext(fragedir_id, () -> fctx.mainvals.new Value(VEC3, new Symbol.Gen("edir")) {
			public Expression root() {
	...
	public static Coord3f obj2view(Coord3f objc, Pipe state, Area view) {
		...
		if (s_loc != null)
			c = s_loc.fin(Matrix4f.id).mul4(c);
		Camera s_cam = state.get(cam);
		if (s_cam != null)
			c = s_cam.fin(Matrix4f.id).mul4(c);
		Projection s_prj = state.get(prj);
		if (s_prj != null)
			c = s_prj.fin(Matrix4f.id).mul4(c);
		float d = 1 / c[3];
		Coord sz = view.sz();
		return (new Coord3f(view.ul.x + ((((c[0] * d) + 1) / 2) * sz.x), view.ul.y + (((-(c[1] * d) + 1) / 2) * sz.y),
				c[2] * d));
	}
