...
public class Resource implements Serializable {
	...
	public static class Pool {
		...
		private class Queued extends Named implements Prioritized, Serializable {
			...
			public Resource get() {
				...
				if (error != null)
					throw (new RuntimeException("Delayed error in resource " + name + " (v" + ver + "), from " + error.src,
							error));
				return (res);
			}
			...
			for (ResSource src : sources) {
				try {
					...
					if (t instanceof LoadException)
						...
						error = new LoadException(String.format("Load error in resource %s(v%d), from %s", res.name, res.ver, src),
								t, null);
					error.src = src;
					if (res.error != null) {
			...
			synchronized (cache) {
				...
				if (cur != null) {
					if ((ver == -1) || (cur.ver == ver)) {
						 ...
						 */
						throw (new LoadException(String.format("Weird version number on %s (%d > %d), loaded from %s", cur.name,
								cur.ver, ver, cur.source), cur));
					}
				}
				synchronized (queue) {
					...
					if (cq != null) {
						if (ver != -1) {
							if (ver < cq.ver) {
								throw (new LoadException(String.format("Weird version number on %s (%d > %d)", cq.name, cq.ver, ver),
										null));
							} else if (ver == cq.ver) {
								cq.boostprio(prio);
								return (cq);
	...
	public class CodeEntry extends Layer {
		...
		public <T> Class<? extends T> getcl(Class<T> cl, boolean fail) {
			...
			if (entry == null)
				throw (new RuntimeException(
						"Tried to fetch non-published res-loaded class " + cl.getName() + " from " + Resource.this.name));
			Class<?> acl;
			synchronized (lpe) {
