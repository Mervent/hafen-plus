	...
	private Object getuval(GLProgram prog, int ui, Pipe pipe) {
		...
		if (val == null)
			throw (new NullPointerException(
					String.format("tried to set null for uniform %s on %s", prog.uniforms[ui], pipe)));
		return (val);
	}
	...
	private void apply2(BGL gl, State[] ns, Pipe to) {
		...
		for (int i = 0; i < pn; i++) {
			...
			if ((slot < GLPipeState.matching.length) && (GLPipeState.matching[slot] != null))
				glpapply(env, gl, GLPipeState.matching[slot], (slot < cur.length) ? cur[slot] : null,
						(slot < ns.length) ? ns[slot] : null);
