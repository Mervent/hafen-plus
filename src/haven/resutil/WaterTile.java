...
public class WaterTile extends Tiler {
	...
	private static final Pipe.Op bcol = new Light.PhongLight(true, new Color(128, 128, 128), new Color(255, 255, 255),
				...
				for (int y = ss.ul.y + 1; y < ss.br.y - 1; y++) {
					for (int x = ss.ul.x + 1; x < ss.br.x - 1; x++) {
						if (s[ss.o(x, y)]) {
							...
							sd[ss.o(x, y)] = ((ed[ss.o(x, y)] * 4) + ed[ss.o(x - 1, y)] + ed[ss.o(x + 1, y)] + ed[ss.o(x, y - 1)]
									+ ed[ss.o(x, y + 1)]) / 8;
						}
					}
				}
	...
	public void model(MapMesh m, Random rnd, Coord lc, Coord gc) {
		...
		if (b.split[s.ts.o(lc)]) {
			...
			s.new Face(b.surf[b.vs.o(lc.x, lc.y + 1)], b.surf[b.vs.o(lc.x + 1, lc.y + 1)], b.surf[b.vs.o(lc.x + 1, lc.y)]);
		}
	}
	...
	static {
		sky = new SamplerCube(
				new RUtils.CubeFill(() -> Resource.local().load("gfx/tiles/skycube").get().layer(Resource.imgc).img).mktex());
	...
	private static final Pipe.Op surfextra = Pipe.Op.compose(new States.DepthBias(2, 2),
			new States.Blending(States.Blending.Factor.ONE, States.Blending.Factor.ONE));
		...
		private ShaderMacro shader = new ShaderMacro() {
			...
			public void modify(final ProgramContext prog) {
								...
								sub(mix(
										...
										add(pick(texture2D(snrm.ref(),
												...
												pick(
														...
														"rgb")),
										abs(sub(Cons.mod(FrameInfo.time(), l(2.0)), l(1.0)))), l(0.5 * 2)),
				...
				Homo3D.frageyen(prog.fctx).mod(in -> {
					...
					return (add(mul(pick(m, "x"), vec3(l(1.0), l(0.0), l(0.0))), mul(pick(m, "y"), vec3(l(0.0), l(1.0), l(0.0))),
							mul(pick(m, "z"), in)));
				}, -10);
				FragColor.fragcol(prog.fctx).mod(
						in -> mul(in,
								textureCube(ssky.ref(),
										neg(mul(icam.ref(),
												reflect(Homo3D.fragedir(prog.fctx).depref(), Homo3D.frageyen(prog.fctx).depref())))),
								l(0.4)),
						0);
			}
		};
	...
	public static class BottomFog extends State.StandAlone {
		...
		/*
		 * XXXRENDER public static final Expression mfogcolor = mul(col3(fogcolor),
		 * pick(fref(idx(ProgramContext.gl_LightSource.ref(), MapView.amblight.ref()),
		 * "diffuse"), "rgb"));
		 */
		...
		private final ShaderMacro shader = prog -> {
			FragColor.fragcol(prog.fctx).mod(in -> rgbmix.call(in, mfogcolor, min(div(fragd.ref(), l(maxdepth)), l(1.0))),
					1000);
		};
	...
	public Pipe.Op drawstate(Glob glob, Coord3f c) {
		/*
		 * XXXRENDER if(cfg.pref.wsurf.val) return(obfog); return(null);
		 */
		return (obfog);
	}
}
