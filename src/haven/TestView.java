...
public class TestView extends PView {
	...
	final Light.PhongLight light = new Light.PhongLight(true, FColor.BLACK, FColor.WHITE, FColor.BLACK, FColor.BLACK,
			0);
	...
	public static class Quad implements Rendered, RenderTree.Node {
		...
		static {
			float[] vert = { 75, 25, 1, 0, 0, 1, 75, 75, 0, 1, 0, 1, 25, 25, 0, 0, 1, 1, 25, 75, 1, 0, 0.5f, 1, };
			VertexArray.Layout fmt = new VertexArray.Layout(
					...
					new VertexArray.Layout.Input(VertexColor.color, new VectorFormat(4, NumberFormat.FLOAT32), 0, 8,
							24));
			VertexArray vao = new VertexArray(fmt,
					new VertexArray.Buffer(vert.length * 4, DataBuffer.Usage.STATIC, DataBuffer.Filler.of(vert)));
