...
public class SkelSprite extends Sprite implements Sprite.CUpd, Skeleton.HasPose {
	public static final Pipe.Op rigid = new BaseColor(FColor.GREEN), morphed = new BaseColor(FColor.RED),
	 ...
	 */
	protected RenderTree.Node animwrap(Pipe.Op.Wrapping wrap, Collection<Runnable> tbuf,
			Collection<Consumer<Render>> gbuf) {
	...
	public void iparts(int mask, Collection<RenderTree.Node> rbuf, Collection<Runnable> tbuf,
			Collection<Consumer<Render>> gbuf) {
	...
	private void chparts(int mask) {
		...
		/*
		 * XXX: Arguably, updating should be forgone if the parts haven't actually
		 * changed. Somewhat ill-defined, however.
		 */
