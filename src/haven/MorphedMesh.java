	...
	public static class MorphedBuf extends VertexBuf {
		...
		private static AttribData[] ohBitterSweetJavaDays(VertexBuf from, Collection<Pair> pos, Collection<Pair> dir,
			...
			for (int i = 0; i < from.bufs.length; i++) {
				MorphType type = (from.bufs[i] instanceof MorphData) ? ((MorphData) from.bufs[i]).morphtype()
						: MorphType.NONE;
		...
		protected VertexArray fmtdata() {
			...
			for (int i = 0; i < fi.length; i++) {
				...
				if (map[bufn] != null) {
					...
					final int fidx = bufn;
					bufs[an] = new VertexArray.Buffer(ni[i].stride * num, DataBuffer.Usage.STREAM,
							(buf, env) -> this.fill(fidx, buf, env));
