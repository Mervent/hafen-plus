...
public class JnlpCache implements ResCache {
	...
	private void put(final URL loc, final byte[] data) {
		/*
		 * Interestingly enough, it seems that the JNLP persistence cache, at least in
		 * Sun's implementation, is REALLY SLOW. "Really slow" meaning that it takes
		 * like 1 second to save each muffin. How they managed to do this, I have no
		 * clue, but to counter it, so as to not take up precious resource/minimap
		 * loader time, the actual storage is deferred.
		 *
		 * Sucks? You don't say.
		 */
	...
	public OutputStream store(final String name) throws IOException {
		/*
		 * The persistence service actually yields a real OutputStream, but since it
		 * needs to know the final size of the data before that, it isn't actually
		 * possible to use it as an OutputStream in any reasonable manner. Thus, all
		 * data is first "pre-cached" in a ByteArrayOutputStream, and then written to
