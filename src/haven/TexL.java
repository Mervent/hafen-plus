...
public abstract class TexL extends TexRender {
	...
	private static Sampler2D mkimg(Coord sz) {
		...
		Texture2D tex = new Texture2D(sz.x, sz.y, DataBuffer.Usage.STATIC, new VectorFormat(4, NumberFormat.UNORM8),
				new VectorFormat(4, NumberFormat.UNORM8), new Filler());
	...
	private class Prepared {
		...
		private Prepared(Environment env) {
			...
			/*
			 * XXXRENDER if((ifmt != null) && (ifmt.nc == 3)) { byte[] pixels =
			 * ((DataBufferByte)img.getRaster().getDataBuffer()).getData();
			 * data.add(pixels); if(mipmap != null) { Coord msz = dim; Mipmapper.Mipmapper3
			 * alg = (Mipmapper.Mipmapper3)mipmap; while((msz.x > 1) || (msz.y > 1)) {
			 * pixels = alg.gen3(msz, pixels, ifmt); data.add(pixels); msz =
			 * Mipmapper.nextsz(msz); } } } else {
			 */
			byte[] pixels;
