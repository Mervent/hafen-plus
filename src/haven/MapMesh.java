...
public class MapMesh implements RenderTree.Node, Disposable {
	...
	private static Pipe.Op gmmat = Pipe.Op.compose(new States.DepthBias(-1, -1), new Order.Default(1001));
		...
		Tiler.MCons cons = new Tiler.MCons() {
			...
			public void faces(MapMesh m, Tiler.MPart d) {
				...
				for (int i = 0; i < d.v.length; i++) {
					...
					texc[i] = new Coord3f((float) (co * texc[i].x + si * texc[i].y), (float) (co * texc[i].y - si * texc[i].x),
							0);
