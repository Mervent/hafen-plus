...
public abstract class TexRender implements Tex, Disposable {
	public static final VertexArray.Layout vf_tex2d = new VertexArray.Layout(
			...
			if ((draw != null) && (clip != null)) {
				if (draw.tex != clip.tex)
					throw (new RuntimeException(
							String.format("TexRender does not support different draw (%s) and clip (%s) textures",
									draw.tex, clip.tex)));
				return (draw.tex.img);
			} else if (draw != null) {
				return (draw.tex.img);
	...
	public void render(GOut g, Coord dul, Coord dbr, Coord tul, Coord tbr) {
		...
		g.usestate(draw);
		g.out.draw(g.state(), new Model(Model.Mode.TRIANGLE_STRIP, new VertexArray(vf_tex2d,
