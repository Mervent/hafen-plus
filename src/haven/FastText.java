...
public class FastText {
	public static final VertexArray.Layout vf = new VertexArray.Layout(
	...
	public static void aprint(GOut g, Coord c, double ax, double ay, String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			short tx1 = (short) (((sx[cc] * 65535) + (ct.tdim.x / 2)) / ct.tdim.x),
					tx2 = (short) (((sx[cc + 1] * 65535) + (ct.tdim.x / 2)) / ct.tdim.x);
		...
		g.out.draw(g.state().copy().prep(ct.st()),
				new Model(Model.Mode.TRIANGLES,
						new VertexArray(vf,
								new VertexArray.Buffer(data.length * 2, DataBuffer.Usage.EPHEMERAL, DataBuffer.Filler.of(data))),
