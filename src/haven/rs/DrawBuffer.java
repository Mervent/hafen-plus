...
public class DrawBuffer implements Disposable {
	...
	public DrawBuffer(Environment env, Coord sz) {
		...
		this.depth = new Texture2D(sz.x, sz.y, DataBuffer.Usage.STATIC, Texture.DEPTH,
				new VectorFormat(1, NumberFormat.FLOAT32), null);
		Area vp = Area.sized(Coord.z, sz);
		this.basic = Pipe.Op.compose(new States.Viewport(vp), new FrameConfig(sz), new FragColor<>(color.image(0)),
				new DepthBuffer<>(depth.image(0)));
	}
	...
	public BufferedImage draw(Pipe.Op state, RenderTree.Node n) {
		...
		Pipe.Op bstate = Pipe.Op.compose(basic(), Homo3D.state, new States.Depthtest(States.Depthtest.Test.LE),
				new States.Facecull(), lights, state);
		basic.ostate(bstate);
