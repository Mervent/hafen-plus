...
public class Phong extends ValBlock.Group {
	...
	public static final Struct s_light = Struct.make(new Symbol.Shared("light"), VEC4, "amb", VEC4, "dif", VEC4, "spc",
			VEC4, "pos", FLOAT, "ac", FLOAT, "al", FLOAT, "aq");
	public static final Struct s_material = Struct.make(new Symbol.Shared("material"), VEC4, "emi", VEC4, "amb", VEC4,
			"dif", VEC4, "spc", FLOAT, "shine");
	...
	public class DoLight extends Function.Def {
			...
			ValBlock.Group tdep = dvals.new Group() {
				...
				public void cons2(Block blk) {
					...
					code.add(new If(eq(pick(fref(ls, "pos"), "w"), l(0.0)),
							new Block(stmt(ass(lvl.tgt, l(1.0))), stmt(ass(dir.tgt, pick(fref(ls, "pos"), "xyz")))),
							new Block(rel.new Def(sub(pick(fref(ls, "pos"), "xyz"), vert)),
									stmt(ass(dir.tgt, normalize(rel.ref()))), dst.new Def(length(rel.ref())),
									stmt(ass(lvl.tgt, inv(add(fref(ls, "ac"), mul(fref(ls, "al"), dst.ref()),
		...
		protected void cons() {
			dvals.cons(code);
			code.add(stmt(aadd(diff, mul(pick(fref(mat, "amb"), "rgb"), pick(fref(ls, "amb"), "rgb"), lvl.ref()))));

			code.add(new If(gt(dl.ref(), l(0.0)), dcalc = new Block()));
			dcalc.add(dcurs = new Placeholder());
			dcalc.add(
					aadd(diff, mul(pick(fref(mat, "dif"), "rgb"), pick(fref(ls, "dif"), "rgb"), dl.ref(), lvl.ref())));

			dcalc.add(new If(gt(shine, l(0.5)), scalc = new Block()));
			svals.cons(scalc);
			scalc.add(scurs = new Placeholder());
			scalc.add(aadd(spec, mul(pick(fref(mat, "spc"), "rgb"), pick(fref(ls, "spc"), "rgb"), sl.ref())));
	...
	private void fmod(final FragmentContext fctx, final Expression bcol, final Expression scol) {
		FragColor.fragcol(fctx)
				.mod(in -> add(mul(in, vec4(bcol, pick(fref(material.ref(), "dif"), "a"))), vec4(scol, l(0.0))), 500);
	}
