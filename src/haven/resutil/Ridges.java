...
public class Ridges extends MapMesh.Hooks {
	...
	private void modelcap(Coord tc, int dir) {
		...
		Vertex[] gv = { ms.fortile(tc.add(tccs[dir])), ms.fortile(tc.add(tccs[(dir + 3) % 4])), ms.new Vertex(close),
				...
				edgec[eo(tc, dir)][edgelc(tc, dir) ? 1 : 0], ms.new Vertex(close), ms.fortile(tc.add(tccs[(dir + 2) % 4])),
	...
	private void modeldiag1(Coord tc, int dir) {
		...
		Vertex[] gv = { ms.fortile(tc.add(tccs[(dir + 1) % 4])), edgec[eo(tc, dir)][edgelc(tc, dir) ? 1 : 0],
				...
				ms.fortile(tc.add(tccs[dir])), ms.fortile(tc.add(tccs[(dir + 3) % 4])), ms.fortile(tc.add(tccs[(dir + 2) % 4])),
				edgec[eo(tc, (dir + 1) % 4)][edgelc(tc, dir) ? 0 : 1], edgec[eo(tc, dir)][edgelc(tc, dir) ? 0 : 1], };
		float[] tcx = new float[8], tcy = new float[8];
		Coord pc = tc.mul(tilesz).mul(1, -1);
		for (int i = 0; i < 8; i++) {
			tcx[i] = clip((gv[i].x - pc.x) / tilesz.x, 0, 1);
	...
	private void modelcomplex(Coord tc, boolean[] breaks) {
		...
		Coord3f[] col;
		{
			...
			Random rnd = m.rnd(tc);
			for (int i = 0; i < n; i++) {
				col[i] = new Coord3f(tcx + ((rnd.nextFloat() - 0.5f) * 5.0f), tcy + ((rnd.nextFloat() - 0.5f) * 5.0f), zs[i]);
			}
		}
		...
		for (int i = 0, d = s; i < 4; i++) {
			if (breaks[d]) {
				ensureedge(tc, (d + 3) % 4);
				ensureedge(tc, d);
				Vertex[] gv = { ms.fortile(tc.add(tccs[d])), edgec[eo(tc, (d + 3) % 4)][edgelc(tc, (d + 3) % 4) ? 1 : 0],
