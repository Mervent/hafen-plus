...
public abstract class BGL {
	...
	public void glBindAttribLocation(final ID program, final ID index, final String name) {
		add(new Command() {
			public void run(GL3 gl) {
				gl.glBindAttribLocation(program.glid(), index.glid(), name);
			}
		});
	}
	...
	public void glTexImage3D(final int target, final int level, final int internalformat, final int width,
			final int height, final int depth, final int border, final int format, final int type, Buffer data) {
		final BufState ds = new BufState(data);
		add(new Command() {
			public void run(GL3 gl) {
				ds.restore();
				gl.glTexImage3D(target, level, internalformat, width, height, depth, border, format, type, ds.buf);
			}
		});
	}
