...
public class RUtils {
	...
	public static abstract class StateTickNode<R extends RenderTree.Node> extends StateNode<R>
			implements TickList.TickNode, TickList.Ticking {
	...
	public static class CubeFill implements DataBuffer.Filler<Image> {
		...
		private static final int[][] deforder = { { 3, 1 }, // +X
				{ 1, 1 }, // -X
				{ 2, 0 }, // +Y
				{ 2, 2 }, // -Y
				{ 2, 1 }, // +Z
		...
		private BufferedImage getsrc(TextureCube tex) {
			...
			if ((ret.getWidth() != ex) || (ret.getHeight() != ey))
				throw (new IllegalArgumentException(
						String.format("cube-texture source size should be (%d, %d), not (%d, %d)", ex, ey,
		...
		public FillBuffer fill(Image gimg, Environment env) {
			...
			if (img.level == 0) {
				...
				buf.pull(ByteBuffer.wrap(TexI.convert(data, new Coord(img.w, img.h),
						new Coord(fc[0] * img.tex.w, fc[1] * img.tex.h), new Coord(img.w, img.h))));
				return (buf);
			}
			return (null);
		}
		...
		public TextureCube mktex() {
			...
			Coord on = osz();
			return (new TextureCube(img.getWidth() / on.x, img.getHeight() / on.y, DataBuffer.Usage.STATIC,
					new VectorFormat(4, NumberFormat.UNORM8), this));
		}
	}
