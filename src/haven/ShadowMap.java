...
public class ShadowMap extends State {
	...
	public ShadowMap(Coord res, float size, float depth, float dthr) {
		lbuf = new Texture2D(res, DataBuffer.Usage.STATIC, Texture.DEPTH, new VectorFormat(1, NumberFormat.FLOAT32),
				null);
		...
		basic = Pipe.Op.compose(new DepthBuffer<>(lbuf.image(0)), new States.Viewport(Area.sized(Coord.z, res)), lproj);
	}
