...
public class GLVertexArray extends GLObject implements BGL.ID {
		...
		env.prepare((GLRender g) -> {
			...
			for (int i = 0; i < mod.va.fmt.inputs.length; i++) {
				...
				if (var != null) {
					...
					if (na == 1) {
						gl.glVertexAttribPointer(var, attr.el.nc, GLRender.glattribfmt(attr.el.cf),
								GLRender.glattribnorm(attr.el.cf), attr.stride, attr.offset);
						gl.glEnableVertexAttribArray(var);
						if (attr.instanced)
							gl.glVertexAttribDivisor(var, 1);
					} else {
						for (int v = 0; v < na; v++) {
							gl.glVertexAttribPointer(var, v, vfmt.nc, GLRender.glattribfmt(vfmt.cf), GLRender.glattribnorm(vfmt.cf),
									attr.stride, attr.offset + (v * vo));
	...
	static class ProgIndex implements Disposable {
		...
		void add(Indexed vao) {
			...
			/*
			 * XXX? It would be nice if VAOs could be cleaned out when programs actually go
			 * away rather than when being re-requested.
			 */
			clean();
		}
