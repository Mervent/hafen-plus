...
public class VorbisStream {
	...
	/**
	 * The number of channels in this Vorbis data. One channel means monaural sound,
	 * and two channels means stereo sound, but do note that other values may occur
	 * as well.
	 */
	...
	/**
	 * Constructs a <code>VorbisStream</code> from a <code>PacketStream</code>. The
	 * constructor will have ensured that the fields containing the Vorbis metadata
	 * have been initialized when it returns.
	 *
	 * @exception java.io.IOException if the <code>PacketStream</code> itself throws
	 ...
	 *
	 * @exception java.io.IOException if the <code>InputStream</code> itself throws
	 *                                an <code>IOException</code>.
	 * @exception FormatException     if a format error is found in the input.
	 */
	 ...
	 * contains one float array for each channel in the Vorbis stream, each of which
	 * contains one float for each sample. Each sample ranges from -1.0 to 1.0.
	 *
	 * <p>
	 * The array is guaranteed to contain as many sample arrays as the {@link #chn}
	 * field indicates. The sample arrays are also guaranteed to be of pairwise
	 * equal length, and to contain at least one sample. Therefore, if
	 ...
	 * {@link #decode()} function to decode data, and encodes it as a 16-bit little
	 * endian byte stream of PCM data. The <code>close</code> function of the
	 * <code>InputStream</code> object will chain to the {@link #close()} function
	 * of this <code>VorbisStream</code> object.
	 *
	...
	/**
	 * This function implements a main function that can be used for either testing
	 * the functionality of this <code>VorbisStream</code> implementation or
	 * individual Ogg/Vorbis files. It takes one command-line argument to be
