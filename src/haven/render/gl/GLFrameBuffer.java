...
public class GLFrameBuffer extends GLObject implements BGL.ID {
		...
		if (color.length > 0) {
			...
			if ((depth != null) && !sz.equals(depth.sz()))
				throw (new IllegalArgumentException(String.format(
						"Framebuffer attachments have differing sizes: color[0]=%s, depth=%s", sz, depth.sz())));
			for (int i = 1; i < color.length; i++) {
				if (!sz.equals(color[i].sz()))
					throw (new IllegalArgumentException(
							String.format("Framebuffer attachments have differing sizes: color[0]=%s, color[i]=%s", sz,
									color[i].sz())));
			}
		} else if (depth != null) {
			this.sz = depth.sz();
	...
	public static Attachment prepimg(GLEnvironment env, Texture.Image<?> img) {
		...
		throw (new IllegalArgumentException(
				String.format("Unsupported image type for framebuffer attachment: %s", img)));
	}
}
