...
public class Charlist extends Widget {
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (button == 1) {
			synchronized (chars) {
				for (int i = 0, y = sau.c.y + sau.sz.y; (i < height)
						&& (i + this.y < chars.size()); i++, y += bg.sz().y + margin) {
			...
			if (args.length > 1) {
				...
				if (rawdesc.length > 3) {
					...
					for (int i = 0; i < rawposes.length; i += 2)
						poses.add(new ResData(ui.sess.getres((Integer) rawposes[i]),
								new MessageBuf((byte[]) rawposes[i + 1])));
				}
