...
public class TexI implements Tex {
	...
	public ColorTex st() {
		...
		if (st == null) {
			synchronized (this) {
				if (this.st == null) {
					Texture2D tex = new Texture2D(tdim.x, tdim.y, DataBuffer.Usage.STATIC,
							new VectorFormat(4, NumberFormat.UNORM8), new VectorFormat(4, NumberFormat.UNORM8),
							(img, env) -> {
								if (img.level != 0)
									return (null);
								...
								if (Utils.eq(tdim, sz) && Utils.eq(detectfmt(back), img.tex.efmt)) {
									buf.pull(ByteBuffer
											.wrap(((DataBufferByte) back.getRaster().getDataBuffer()).getData()));
								} else {
	...
	public void render(GOut g, Coord dul, Coord dbr, Coord tul, Coord tbr) {
		...
		float[] data = { dbr.x, dul.y, tr, tu, dbr.x, dbr.y, tr, tb, dul.x, dul.y, tl, tu, dul.x, dbr.y, tl, tb, };
		g.usestate(st());
	...
	/*
	 * Java's image model is a wee bit complex, so these may not be entirely
	 * correct.
	 */
	public static VectorFormat detectfmt(BufferedImage img) {
		...
		if ((cm.getNumComponents() == 4) && (off.length == 4)) {
			if (((cs[0] == 8) && (cs[1] == 8) && (cs[2] == 8) && (cs[3] == 8))
				...
				/*
				 * XXXRENDER: Support component swizzling: if((off[0] == 2) && (off[1] == 1) &&
				 * (off[2] == 0) && (off[3] == 3)) return(GL.GL_BGRA);
				 */
			}
