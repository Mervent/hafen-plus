		...
		public class CPanel extends Widget {
			...
			public CPanel(GSettings gprefs) {
				...
				add(new Label("Render scale"), new Coord(0, y));
				{
					...
					final int steps = 4;
					add(new HSlider(160, -2 * steps, 2 * steps,
				...
				add(new Label("Framerate limit (active window)"), new Coord(0, y));
				{
					Label dpy = add(new Label(""), new Coord(165, y + 15));
					final int max = 250;
					add(new HSlider(160, 1, max,
							(prefs.hz.val == Float.POSITIVE_INFINITY) ? max : prefs.hz.val.intValue()) {
						protected void added() {
							dpy();
							this.c.y = dpy.c.y + ((dpy.sz.y - this.sz.y) / 2);
						}
				...
				add(new Label("Framerate limit (background window)"), new Coord(0, y));
				{
					...
					final int max = 250;
					add(new HSlider(160, 1, max,
							(prefs.bghz.val == Float.POSITIVE_INFINITY) ? max : prefs.bghz.val.intValue()) {
						protected void added() {
							dpy();
							this.c.y = dpy.c.y + ((dpy.sz.y - this.sz.y) / 2);
						}
				...
				y += 15;
				{
					...
					RadioGroup grp = new RadioGroup(this) {
						public void changed(int btn, String lbl) {
							...
							try {
								ui.setgprefs(
										prefs = prefs.update(null, prefs.syncmode, JOGLPanel.SyncMode.values()[btn]));
							} catch (GSettings.SettingException e) {
								error(e.getMessage());
								return;
							}
				...
				/*
				 * XXXRENDER add(new CheckBox("Antialiasing") { {a = cf.fsaa.val;}
				 *
				 * public void set(boolean val) { try { cf.fsaa.set(val); }
				 * catch(GLSettings.SettingException e) { error(e.getMessage()); return; } a =
				 * val; cf.dirty = true; } }, new Coord(0, y)); y += 25; add(new
				 * Label("Anisotropic filtering"), new Coord(0, y)); if(cf.anisotex.max() <= 1)
				 * { add(new Label("(Not supported)"), new Coord(15, y + 15)); } else { final
				 * Label dpy = add(new Label(""), new Coord(165, y + 15)); add(new HSlider(160,
				 * (int)(cf.anisotex.min() * 2), (int)(cf.anisotex.max() * 2),
				 * (int)(cf.anisotex.val * 2)) { protected void added() { dpy(); this.c.y =
				 * dpy.c.y + ((dpy.sz.y - this.sz.y) / 2); } void dpy() { if(val < 2)
				 * dpy.settext("Off"); else dpy.settext(String.format("%.1f\u00d7", (val /
				 * 2.0))); } public void changed() { try { cf.anisotex.set(val / 2.0f); }
				 * catch(GLSettings.SettingException e) { error(e.getMessage()); return; }
				 * dpy(); cf.dirty = true; } }, new Coord(0, y + 15)); }
				 */
