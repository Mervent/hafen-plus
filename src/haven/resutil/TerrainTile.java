...
public class TerrainTile extends Tiler implements Tiler.MCons, Tiler.CTrans {
				...
				for (int x = es.ul.x; x < es.br.x; x++) {
					...
					for (int i = var.length; i >= 0; i--) {
						if (fall) {
							...
							en[i][es.o(x, y)] = true;
							if ((bv[i][vs.o(x, y)] > 0.99f) && (bv[i][vs.o(x + 1, y)] > 0.99f) && (bv[i][vs.o(x, y + 1)] > 0.99f)
									&& (bv[i][vs.o(x + 1, y + 1)] > 0.99f)) {
								fall = true;
							}
						}
					}
	...
	public static class RidgeTile extends TerrainTile implements Ridges.RidgeTile {
		...
		public RidgeTile(int id, SNoise3 noise, NodeWrap base, Var[] var, Tileset transset, int rth, Pipe.Op rmat,
				float texh) {
