	...
	public abstract class Equ<R extends RenderTree.Node> extends RUtils.StateNode<R> {
		...
		private Equ(R r, ED ed) {
			...
			if ((bt == null) && !ed.at.equals(""))
				throw (new RuntimeException("Transformation " + ed.at + " for equipment " + ed.res + " on skeleton " + skel
				...
				creat: {
					for (Equ equ : this.equ) {
						if (equ.desc.equals(ed)) {
							ne = equ;
							break creat;
						} else if ((equ instanceof SpriteEqu) && (((SpriteEqu) equ).r instanceof Sprite.CUpd)
								&& equ.desc.equals2(ed)) {
							/*
							 * XXX: This is impure and ugly, but fixing it properly would seem to be
							 * significantly more complex for what is probably little benefit.
							 */
