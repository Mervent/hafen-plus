...
public class TextureCube extends Texture {
	...
	public TextureCube(int w, int h, DataBuffer.Usage usage, VectorFormat ifmt, VectorFormat efmt,
			DataBuffer.Filler<? super Image> init) {
		super(usage, ifmt, efmt, init);
		if ((w < 0) || (h < 0))
			throw (new IllegalArgumentException(
					String.format("Texture sizes must be non-negative, not (%d, %d)", w, h)));
		this.w = w;
		this.h = h;
	...
	public TextureCube(Coord dim, DataBuffer.Usage usage, VectorFormat ifmt, VectorFormat efmt,
			DataBuffer.Filler<? super Image> init) {
	...
	public CubeImage image(Face face, int level) {
		...
		if (level > 0) {
			...
			if (((w >> level) == 0) && ((h >> level) == 0))
				throw (new IllegalArgumentException(
						String.format("Invalid mipmap level %d for (%d, %d) texture", level, w, h)));
		}
		...
		return (new AbstractCollection<CubeImage>() {
			private int msize() {
				if ((w == 0) || (h == 0)) {
					...
					return (Math.max(Integer.numberOfTrailingZeros(w), Integer.numberOfTrailingZeros(h)) + 1);
				} else {
					return (1);
				}
			}
