...
public class MapView extends PView implements DTarget, Console.Directory {
	...
	public class FollowCam extends Camera {
		...
		public void tick(double dt) {
			...
			view = new haven.render.Camera(
					PointedCam.compute(curc.add(camoff).add(0.0f, 0.0f, h), dist(elev), elev, angl));
			proj = new Projection(
					Projection.makefrustum(new Matrix4f(), -field, field, -ca * field, ca * field, 1, 5000));
		}
	...
	public class OrthoCam extends Camera {
		...
		public void tick(double dt) {
			...
			proj = new Projection(
					Projection.makeortho(new Matrix4f(), -field, field, -field * aspect, field * aspect, 1, 5000));
		}
		...
		private void addgob(Gob ob) {
			...
			synchronized (ob) {
				...
				try {
					...
					/*
					 * Ignore here as there is a harmless remove-race on disposal.
					 */
					return;
				}
				synchronized (this) {
	...
	private class MapRaster extends RenderTree.Node.Track1 {
		...
		abstract class Grid<T> extends RenderTree.Node.Track1 {
			...
			void tick() {
				...
				for (Coord cc : area) {
					try {
						...
						if ((cur != null) && (cur.a != cut)) {
							/*
							 * XXX: It is currently important that invalidated cuts are removed immediately
							 * (since they are disposed in MCache in thus not drawable anymore). This is not
							 * currently a problem, but conflicts with the below stated goal of
				...
				for (Iterator<Map.Entry<Coord, Pair<T, RenderTree.Slot>>> i = cuts.entrySet().iterator(); i
						.hasNext();) {
		...
		void tick() {
			/*
			 * XXX: Should be taken out of the main rendering loop. Probably not a big deal,
			 * but still.
			 */
			try {
	...
	private void amblight() {
		synchronized (glob) {
			if (glob.lightamb != null) {
				amblight = new DirLight(glob.lightamb, glob.lightdif, glob.lightspc,
	...
	public static class Clicklist implements RenderList<Rendered>, RenderList.Adapter {
		public static final Pipe.Op clickbasic = Pipe.Op.compose(new States.Depthtest(States.Depthtest.Test.LE),
				new States.Facecull(), Homo3D.state);
		...
		public static int col2id(Color col) {
			...
			return (((r & 0xf0) >> 4) | ((r & 0x0f) << 12) | ((g & 0xf0) >> 0) | ((g & 0x0f) << 16) | ((b & 0xf0) << 4)
					| ((b & 0x0f) << 20));
		}
		...
		public void get(Render out, Coord c, Consumer<ClickData> cb) {
			GOut.getpixel(out, basic, FragID.fragid, c, col -> {
				...
				cb.accept(new ClickData(cs.bk.state().get(Clickable.slot),
						(RenderTree.Slot) cs.bk.cast(RenderTree.Node.class)));
			});
		}
	...
	private Pipe.Op clickbasic(Coord sz) {
		if ((curclickbasic == null) || !clickid.image.tex.sz().equals(sz)) {
			...
			clickid = new FragID<>(
					new Texture2D(sz, DataBuffer.Usage.STATIC, new VectorFormat(4, NumberFormat.UNORM8), null)
							.image(0));
			clickloc = new ClickLocation<>(
					new Texture2D(sz, DataBuffer.Usage.STATIC, new VectorFormat(4, NumberFormat.UNORM8), null)
							.image(0));
			clickdepth = new DepthBuffer<>(new Texture2D(sz, DataBuffer.Usage.STATIC, Texture.DEPTH,
					new VectorFormat(1, NumberFormat.FLOAT32), null).image(0));
			curclickbasic = Pipe.Op.compose(Clicklist.clickbasic, clickid, clickdepth,
					new States.Viewport(Area.sized(Coord.z, sz)));
		}
	...
	public abstract class Maptest {
		...
		public void run() {
			...
			checkmapclick(out, basic, pc, mc -> {
				/*
				 * XXX: This is somewhat doubtfully nice, but running it in the defer group
				 * would cause unnecessary latency, and it shouldn't really be a problem.
				 */
				new HackThread(() -> {
					synchronized (ui) {
						if (mc != null)
