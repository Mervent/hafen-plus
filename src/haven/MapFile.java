...
public class MapFile {
	...
	public static class ZoomGrid extends DataGrid {
		...
		public static ZoomGrid from(MapFile file, Segment seg, int lvl, Coord sc) {
			...
			/*
			 * XXX: This is hardly "correct", but the correct implementation would require a
			 * topological sort, and it's not like it really matters that much.
			 */
			int nt = 0;
			TileInfo[] infos;
		...
		public static ZoomGrid load(MapFile file, long seg, int lvl, Coord sc) {
			...
			try (StreamMessage data = new StreamMessage(fp)) {
				...
				if (ver == 1) {
					...
					if (storedseg != seg)
						throw (new Message.FormatError(
								String.format("Zoomgrid segment mismatch: expected %s, got %s", seg, storedseg)));
					long storedlvl = z.int32();
					if (storedlvl != lvl)
						throw (new Message.FormatError(
								String.format("Zoomgrid level mismatch: expected %s, got %s", lvl, storedlvl)));
					Coord storedsc = z.coord();
					if (!sc.equals(storedsc))
						throw (new Message.FormatError(
								String.format("Zoomgrid coord mismatch: expected %s, got %s", sc, storedsc)));
					...
					throw (new Message.FormatError(String.format(
							"Unknown zoomgrid data version for (%d, %d) in %x@%d: %d", sc.x, sc.y, seg, lvl, ver)));
				}
			} catch (Message.BinError e) {
